...
def run():
    ...
    # credentials.txt需自己输入账号密码，“,”隔开，一行一组
    # share_tokens.txt记录产生的对应的Pandora的share token和/或pool token。
    ...
    for credential in credentials:
        ...
        try:
            ...
            data = {'username': username,
                    'password': password,
                    'prompt': 'login',}
            resp = requests.post('https://ai-20230626.fakeopen.com/auth/login', data=data)
            if resp.status_code == 200:
                feedback_info= resp.json()
                token_info['token'] =feedback_info['access_token']
            #print(auth)
